    ...
    while True:
        ...
        if 'hey lola' in pedido:
            ...
            while comenzar:
                ...
                elif 'whatsapp' in pedido or 'Whatsapp':
                    hablar('¿A que numero deseas enviarlo?')
                    numero = '+34'+transfAudio().replace(' ','')
                    hablar('¿Que quieres decir?')
                    mensaje = transfAudio()
                    hablar(mensaje + '. ¿Es correcto?')
                    try:
                        pywhatkit.sendwhatmsg_instantly(numero, mensaje,1)
                    except:
                        continue
